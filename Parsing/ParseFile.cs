    ...
    public class ParseFile
    {
        public static Instructions[] BeginParse(string file)
        {
            ...
            foreach (string line in lines)
            {
                ...
                else if (line.Contains("add-header"))
                {
                    string data = GetData(line.Remove(0, 10));
                    Instructions newInstruct = new Instructions(InstructionType.AddHeader, data);
                    instructions.Add(newInstruct);
                }
                else if (line.Contains("include-directory"))
                {
                    string data = GetData(line.Remove(0, 17));
                    Instructions newInstruct = new Instructions(InstructionType.AddIncludeDirectory, data);
                    instructions.Add(newInstruct);
                }
